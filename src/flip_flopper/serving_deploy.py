"""Build and apply Databricks Model Serving endpoint config for four UC ONNX models."""
...
def registered_model_fqn(catalog: str, schema: str, normalized_suffix: str, letter: str) -> str:
    ...
    if letter_l not in {"a", "b", "x", "l"}:
        msg = "letter must be 'a', 'b', 'x', or 'l'"
        raise ValueError(msg)
...
def build_serving_config(
    *,
    ...
    entity_name_c: str = "classifier_catboost",
    entity_name_d: str = "classifier_lightgbm",
    traffic_pct_a: int = 25,
    traffic_pct_b: int = 25,
    traffic_pct_c: int = 25,
    traffic_pct_d: int = 25,
    workload_size: str = "Small",
    scale_to_zero: bool = True,
) -> dict[str, Any]:
    if traffic_pct_a + traffic_pct_b + traffic_pct_c + traffic_pct_d != 100:
        msg = (
            "traffic percentages must sum to 100, "
            f"got {traffic_pct_a} + {traffic_pct_b} + {traffic_pct_c} + {traffic_pct_d}"
        )
        raise ValueError(msg)
    va = str(int(version_a))
    vb = str(int(version_b))
    vc = str(int(version_c))
    vd = str(int(version_d))
    return {
        "served_entities": [
            {
                "name": entity_name_a,
        ...
        "traffic_config": {
            "routes": [
                ...
                {
                    "served_model_name": entity_name_c,
                    "traffic_percentage": traffic_pct_c,
                },
                {
                    "served_model_name": entity_name_d,
                    "traffic_percentage": traffic_pct_d,
                },
            ]
        },
    }
...
def deploy_endpoint(
    *,
    ...
    alias_c: str = "Champion",
    alias_d: str = "Champion",
    traffic_pct_a: int = 25,
    traffic_pct_b: int = 25,
    traffic_pct_c: int = 25,
    traffic_pct_d: int = 25,
    ...
    model_c = registered_model_fqn(catalog, schema, normalized, letter_c)
    model_d = registered_model_fqn(catalog, schema, normalized, letter_d)
    ...
    v_c = resolve_version_for_alias(mclient, model_c, alias_c)
    v_d = resolve_version_for_alias(mclient, model_d, alias_d)
    ...
    config = build_serving_config(
        ...
        entity_name_d=entity_name_d,
        traffic_pct_a=traffic_pct_a,
        traffic_pct_b=traffic_pct_b,
        traffic_pct_c=traffic_pct_c,
        traffic_pct_d=traffic_pct_d,
        workload_size=workload_size,
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description="Create or update a Databricks serving endpoint for four UC ONNX classifiers."
    )
    ...
    p.add_argument("--alias-c", default="Champion")
    p.add_argument("--alias-d", default="Champion")
    p.add_argument("--traffic-a", type=int, default=25)
    p.add_argument("--traffic-b", type=int, default=25)
    p.add_argument("--traffic-c", type=int, default=25)
    p.add_argument("--traffic-d", type=int, default=25)
    ...
    p.add_argument("--entity-name-c", default="classifier_catboost")
    p.add_argument("--entity-name-d", default="classifier_lightgbm")
    p.add_argument("--letter-c", default="x")
    p.add_argument("--letter-d", default="l")
    ...
    print(
        ...
        f"{result['model_c']}@{result['version_c']} ({ns.alias_c}) / "
        f"{result['model_d']}@{result['version_d']} ({ns.alias_d})"
    )
    return 0
